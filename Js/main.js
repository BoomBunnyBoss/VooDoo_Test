  ...
  .then(data => {
    ...
    if (data && data.products && data.products.length > 9) {
      ...
      products.forEach((product, index) => {
        ...
        const imageContainer = document.getElementById(`${productIndex}-image-container`);
        const titleElement = document.getElementById(`${productIndex}-title`);
        const priceElement = document.getElementById(`${productIndex}-price`);

        if (product.images && product.images.length > 0) {
          const imageSrc = product.images[0].src;

          if (productIndex === 10 || productIndex === 11 || productIndex === 12) {
            // Вывести только первое изображение продукта 10, 11 и 12
            imageContainer.src = imageSrc;
          }
        } else {
          console.log(`У продукта ${productIndex} нет изображений`);
        }
